...
def load_from_db():
    ...
    global hryaky_data, stats_data, warns_data, spam_data, manual_users
    
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    
    # Завантажуємо хряків
    cursor.execute('SELECT * FROM hryaky')
    for row in cursor.fetchall():
        ...
        hryaky_data[key] = {
            ...
            'max_gain': row[11] if row[11] is not None else 0,
            'max_gains_20': row[12] if row[12] is not None else 0,
    ...
    for row in cursor.fetchall():
        key = row[0]
        manual_users[row[2]] = json.loads(row[3]) if row[3] else []
...
/warn — вида��и попередження (відповідь + /warn причина)
